...
Ignition uses a JSON format that is specified in several versions: v1, v2 and v3. While v1 and v2 are still supported in Flatcar Container Linux, it's recommended to write new configuration with v3.
...
* [butane][butane-spec] can be used to generate Ignition v3 specification
...
Ignition v2 is not developed anymore but still supported (specification 2.3.0), [ct][ct-config] can be used to emit Ignition v2 spec:
